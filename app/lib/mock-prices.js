// أسعار احتياطية واقعية تُستخدم عندما لا تتوفر بيانات لحظية في قاعدة البيانات أو من Yahoo Finance.
// نفس المصدر يُستخدم في كل مكان (السعر الحالي والرسم البياني التاريخي) حتى تبقى متّسقة.
...
// نفس السعر الاحتياطي المعروض في بطاقة السهم، ليبقى الرسم البياني متّسقاً معه دائماً.
export function getMockPrice(symbolParam) {
    const symbol = (symbolParam || '').replace('.CA', '');
    const entry = mockPrices[symbol] || generateMockData(symbol);
